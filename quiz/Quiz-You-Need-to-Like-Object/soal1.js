...
function ganjilGenapAsianGames(date, data) {
  ...
  for(i = 0; i < data.length; i++){
    ...
    if(vehicle.type === 'Mobil'){
      let plat = vehicle.plat;
      let platNum = plat.match(/\d+/g);

      if(platNum && platNum.length > 0){
        let lastNum = platNum[platNum.length - 1];
        let lastDigitNum = parseInt(lastNum[lastNum.length - 1]);
        let isPlatEven = lastDigitNum % 2 === 0;
